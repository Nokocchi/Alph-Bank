...
import static com.alphbank.corepaymentservice.LogUtils.legalLogMediaTypes;
import static com.alphbank.corepaymentservice.LogUtils.loggingRequest;
...
public class PartnerServerHttpRequestDecorator extends ServerHttpRequestDecorator {
    ...
    PartnerServerHttpRequestDecorator(ServerHttpRequest delegate) {
        ...
        if (true || legalLogMediaTypes.contains(contentType)) {
            body = flux.publishOn(single()).map(dataBuffer -> loggingRequest(log, dataBuffer));
        } else {
            body = flux;
        }
    }
